...
export default function BotButtonsPanel({
  ...
  const [isFreeTestActive, setIsFreeTestActive] = useState(settings.isFreeTestActive !== undefined ? settings.isFreeTestActive : true);
  const [freeTestDisabledMessage, setFreeTestDisabledMessage] = useState(settings.freeTestDisabledMessage || "اکانت تست رایگان فعلا موجود نیست.");
  const [showFreeTestMessageConfig, setShowFreeTestMessageConfig] = useState(false);
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in font-sans">
          <div className="bg-[#111827] border border-slate-700/60 p-6 rounded-2xl w-full max-w-lg shadow-[0_0_40px_rgba(0,0,0,0.5)]">
            <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
              ...
              {lang === "fa" ? "غیرفعال‌سازی سرویس تست رایگان" : "Disable Free Test Service"}
            </h3>
            ...
            <textarea
              ...
              placeholder={lang === "fa" ? "مثلا: اکانت تست رایگان فعلا موجود نیست." : "e.g., Free test is not available right now."}
            />
